...
Решение.
Основой решения является интерфейс Filter, реализации которого представляют собой фильтры для различных 
ситуаций. Таким образом система получается расширяемой в последствии. Для каждой реализации представлен 
тестирующий класс на JUnit, который проверяет работоспособность функционала.

Ответы на теоретические вопросы представлены в папке Questions в файле questionary.odt.
